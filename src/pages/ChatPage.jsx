...
const ChatPage = () => {
        ...
        const fetchProducts = async () => {
            ...
            for (const conv of conversations) {
                if (conv.auctionId && !newProductsMap[conv.auctionId]) {
                    try {
                        const auction = await auctionService.getAuctionById(conv.auctionId);
                        newProductsMap[conv.auctionId] = auction;
                        hasChanges = true;
                    } catch (err) {
    ...
    return (
        <div className="flex h-[calc(100vh-64px)] bg-gradient-to-br from-blue-50 via-white to-blue-50 overflow-hidden font-sans">
            {/* Left Sidebar: Conversations List */}
            <div className="w-1/4 min-w-[300px] bg-white/70 backdrop-blur border-r border-blue-100 flex flex-col">
                {/* Header */}
                <div className="p-4 border-b border-blue-100">
                    ...
                    <div className="relative">
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 bg-white/70 border border-blue-100 rounded-xl text-sm focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-200"
                        />
                ...
                {/* Tabs */}
                <div className="flex px-4 py-2 gap-2 border-b border-blue-100 overflow-x-auto no-scrollbar">
                    <button className="px-3 py-1 bg-blue-600 text-white text-sm rounded-full whitespace-nowrap">Tất cả</button>
                    <button className="px-3 py-1 bg-white/70 text-gray-700 text-sm rounded-full whitespace-nowrap hover:bg-white border border-blue-100">Chưa đọc</button>
                    <button className="px-3 py-1 bg-white/70 text-gray-700 text-sm rounded-full whitespace-nowrap hover:bg-white border border-blue-100">Tin rác / Bỏ qua</button>
                </div>
                        ...
                        conversations.map(conv => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`p-3 border-b border-blue-50 hover:bg-white/70 cursor-pointer transition-colors flex gap-3 group ${isActive ? 'bg-blue-50' : ''}`}
                                >
                                    <div className="relative">
            ...
            {/* Chat Area */}
            <div className="flex-1 flex flex-col bg-white/70 backdrop-blur relative">
                {!activeConversation ? (
                    ...
                    <>
                        {/* 1. Header */}
                        <div className="h-16 px-4 border-b border-blue-100 flex items-center justify-between shrink-0 bg-white/50">
                        ...
                        {activeProduct && (
                            <div className="bg-white/60 backdrop-blur p-3 flex items-center gap-4 border-b border-blue-100 shrink-0 sticky top-16 z-10">
                                <div className="w-12 h-12 bg-white rounded-xl border border-blue-100 p-0.5 shrink-0 overflow-hidden">
                                    <img
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 min-w-0">
                                        <h4 className="font-semibold text-gray-900 truncate">{activeProduct.title}</h4>
                                        {activeProduct.status != null && (
                                            <span className={`shrink-0 text-[11px] px-2 py-0.5 rounded-full border ${
                                                activeProduct.status === 1
                                                    ? 'bg-green-50 text-green-700 border-green-200'
                                                    : activeProduct.status === 3
                                                        ? 'bg-blue-50 text-blue-700 border-blue-200'
                                                        : 'bg-gray-50 text-gray-700 border-gray-200'
                                            }`}>
                                                {statusNames[activeProduct.status] ?? '—'}
                                            </span>
                                        )}
                                    </div>
                                    <p className="text-blue-700 font-bold text-sm">
                                        {(activeProduct.currentPrice ?? activeProduct.startingPrice)?.toLocaleString?.('vi-VN')} ₫
                                    </p>
                                </div>
                                {activeConversation.auctionId && (
                                    <Link
                                        to={`/auction/${activeConversation.auctionId}`}
                                        className="shrink-0 text-sm font-semibold px-3 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700 transition-colors"
                                    >
                                        Xem chi tiết
                                    </Link>
                                )}
                        ...
                        <div className="shrink-0 border-t border-blue-100 bg-white/70 backdrop-blur">
                            {/* Quick Chips Carousel */}
                            <div className="flex gap-2 overflow-x-auto p-2 no-scrollbar border-b border-blue-50">
                                {quickReplies.map((reply, i) => (
                                    <button
                                        key={i}
                                        onClick={() => sendQuickReply(reply)}
                                        className="px-3 py-1.5 bg-white/70 hover:bg-white rounded-full text-xs text-gray-700 whitespace-nowrap transition-colors border border-blue-100"
                                    >
                                        {reply}
                                    </button>
                                ))}
                            ...
                            <div className="p-3">
                                ...
                                <form onSubmit={handleSend} className="relative flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        className="p-2 text-gray-500 hover:bg-white rounded-full"
                                    >
                                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                                    </button>
                                    <button type="button" className="p-2 text-gray-500 hover:bg-white rounded-full">
                                    ...
                                    <div className="flex-1 relative">
                                        <input
                                            ...
                                            className="w-full pl-4 pr-10 py-2.5 bg-white/70 border border-blue-100 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-300 focus:bg-white transition-all"
                                        />
                                        <button
                                            type="button"
                                    ...
                                    {newMessage.trim() ? (
                                        <button
                                            type="submit"
                                            className="p-2.5 bg-blue-600 hover:bg-blue-700 rounded-full text-white shadow-sm transition-transform transform active:scale-95"
                                        >
