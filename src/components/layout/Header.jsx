...
const Header = () => {
  ...
  const adminNavLinks = [
    ...
    { to: '/admin/products', label: 'Sản phẩm', icon: ICONS.tag },
            ...
            <nav style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', flex: 1 }}>
              {adminNavLinks.map(({ to, label }) => (
                <Link
                  key={to}
                  to={to}
                  className={`hdr-nav-link-admin${isActive(to) ? ' active' : ''}`}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
              ...
              <div style={{ position: 'relative' }} ref={notificationRef}>
                <button
                  ...
                  className="hdr-icon-btn hdr-icon-btn-admin"
                  aria-label="Thông báo"
                >
                  <Icon d={ICONS.bell} size={18} />
                  {notificationUnread > 0 && (
                    <span className="hdr-badge hdr-badge-admin">
              ...
              <div style={{ position: 'relative' }} ref={dropdownRef}>
                <button
                  ...
                  className="hdr-avatar-btn hdr-avatar-btn-admin"
                >
                  <div className="hdr-avatar hdr-avatar-admin">
                    {getInitials(user?.fullName)}
                  </div>
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'rgba(250,250,249,0.9)', lineHeight: 1.2, maxWidth: 100, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {user?.fullName || 'Admin'}
                    </div>
                  </div>
                  <div style={{ color: 'rgba(250,250,249,0.4)', transition: 'transform 0.2s', transform: dropdownOpen ? 'rotate(180deg)' : 'none' }}>
                    <Icon d={ICONS.chevron} size={14} strokeWidth={2} />
                  </div>
                </button>
                {dropdownOpen && (
                  <div className="hdr-dropdown hdr-dropdown-admin">
                    <AdminDropdown
                      user={user}
                      onProfile={() => { navigate('/profile'); setDropdownOpen(false); }}
                      onHome={() => { navigate('/'); setDropdownOpen(false); }}
